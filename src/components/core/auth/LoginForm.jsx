...
const LoginForm=()=>{
    ...
    return (
        <div className="h-screen  flex flex-col overflow-x-hidden  items-center justify-center w-full     mx-auto ">
            ...
            <Formik  onSubmit={(values)=>(handleSubmit(values))} initialValues={{email:"",password:""}} >
                <Form className="mt-8 w-full px-3 max-w-[30rem]  overflow-hidden ">
                 <div className="flex  flex-col gap-8">

                    <Field  required className="outline-offset-0 bg-white/10  outline-none p-3 w-full rounded-md font-semibold text-xl sm:text-3xl" placeholder="Enter email address" name="email" type="email" />
                    
                    <Field required className="bg-white/10 p-3 outline-offset-0   w-full outline-none rounded-md text-xl sm:text-3xl font-semibold" placeholder="Enter password" name="password" type="password" />
                 </div>
