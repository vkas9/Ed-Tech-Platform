...
const ForgotPassword = () => {
  ...
  return (
    ...
    transition={{ duration: 0.4, delay: 0.2, ease: [0, 0.71, 0.2, 1.01] }} className="h-screen  relative flex flex-col overflow-x-hidden items-center justify-center w-full mx-auto">
      ...
      <div>
        <Formik onSubmit={(value)=>handleSubmit(value)} initialValues={{ email: "" }}>
          <Form className="mt-8 w-full backdrop-blur-sm px-3  max-w-[40rem]  overflow-hidden ">
            ...
            <button disabled={loading}  className={`bg-yellow-500  hover:bg-yellow-600 text-yellow-950   transition-all duration-150 w-full font-bold text-2xl ${loading?"opacity-50 cursor-not-allowed":" "} active:bg-yellow-600 p-2 rounded-md mt-3  `} type="submit">{loading ? "Sending..." : "Send Code"}</button>
          </Form>
        </Formik>
      </div>
    </motion.div>
