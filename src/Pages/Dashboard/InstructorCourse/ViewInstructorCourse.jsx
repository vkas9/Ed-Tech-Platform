...
import VideoModal from "../EnrolledCourse/VideoModal/VideoModal";
const ViewInstructorCourse = () => {
  const { allInstructoreCourses } = useSelector((store) => store.course);
  ...
  return (
    ...
    >
      ...
      <Link to={"/"} className="underline active:text-white  sm:hover:text-white ">Home</Link>
        ...
        <span>/</span>
        <Link to={"/dashboard/my-courses"} className="whitespace-nowrap underline active:text-white  sm:hover:text-white ">My Courses</Link>
        <span>/</span>
        ...
        {eCourse?.Section.map((section) => (
          <details key={section._id} className="mb-2">
            <summary
              ...
              className="cursor-pointer flex items-center justify-between px-2 py-3 bg-white/20 rounded-md relative hover:bg-white/30"
            >
