...
const ViewCourse = () => {
    ...
    useEffect(() => {
        (async () => {
            const handle = await get("video-directory-handle");
            if (handle) {
    ...
    useEffect(() => {
        if (!directoryHandle || downloadedVideos.length === 0) return;
        (async () => {
            const newURLs = {};
            for (const id of downloadedVideos) {
                try {
                    ...
                    console.warn(`Cannot access file ${id}.mp4`);
                }
            }
            setLocalVideoURLs(newURLs);
        })();
    }, [directoryHandle, downloadedVideos]);
  ...
  if (!eCourse) {
    ...
    const startDownload = async (video_item) => {
        setDownloadingVideos((p) => [...p, video_item._id]);
        try {
            ...
            if (!dir) {
                ...
                if (perm === "granted") {
                    ...
                    await set("video-directory-handle", dir);
            ...
            const fileName = `${video_item._id}.mp4`;
            ...
            setLocalVideoURLs((p) => ({ ...p, [video_item._id]: url }));
            setDownloadedVideos((p) => [...p, video_item._id]);
        } catch (e) {
            console.error("Download failed:", e);
        } finally {
            setDownloadingVideos((p) =>
                p.filter((id) => id !== video_item._id)
            );
        }
    };
        ...
        >
            <div className="flex gap-1 text-white/50 scrollbar scrollbar-thumb-scrollbar-thumb scrollbar-track-scrollbar-bg scrollbar-thumb-rounded-full scrollbar-track-rounded-full overflow-x-auto">
                ...
                <Link
                    ...
                    className="underline active:text-white sm:hover:text-white whitespace-nowrap"
                >
            ...
            <h2 className="text-white/50 mb-1">
                {eCourse.Section.length} Sections • {getTotalLectures()}{" "}
                Lectures
            </h2>
                    ...
                    <details key={section._id} className="mb-2">
                        ...
                        <div>
                            {section?.subSection?.map((subsection) => {
                                const isDL = downloadedVideos.includes(
                                    subsection._id
                                );
                                const src = isDL
                                    ? localVideoURLs[subsection._id]
                                    : subsection.videoURL;
                                return (
                                    <div
                                        key={subsection._id}
                                    ...
                                    >
                                        ...
                                        <div className="px-4 hover:bg-white/20 hover:cursor-pointer relative py-3 bg-white/10 rounded-md ml-4 mt-2">
                                            {downloadedVideos.includes(
                                                subsection._id
                                            ) ? (
                                                <MdOutlineDownloadDone className="min-w-[50px]" />
                                            ) : downloadingVideos.includes(
                                                  subsection._id
                                              ) ? (
                                                <img
                                                    src={loading}
